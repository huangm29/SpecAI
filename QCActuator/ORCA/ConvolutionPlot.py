...
# SpecType: number; type of spectra; 1: XAS; 2: UV-vis; 3: Vibrational (IR and Raman)
...
##################################################################################################
    # Plot NMR spectrum
    if SpecType == 4: 
        with open("hydrogen_data.json", "r") as json_file:
            hydrogen_data = json.load(json_file)

            ppm = [row[0] for row in hydrogen_data]
            intensity = [row[1] for row in hydrogen_data]
        
            plt.figure(figsize=(10, 6))
            plt.vlines(ppm, [0], intensity, colors="black")
            plt.gca().invert_xaxis()
            plt.title(PlotTitle)
            plt.xlabel("Chemical Shift (ppm)")
            plt.ylabel("Intensity")
            plt.ylim((0, max(intensity) * 1.2))
            plt.show()
